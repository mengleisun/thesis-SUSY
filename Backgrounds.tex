...
\begin{table}[hbt]
  ...
  \begin{tabular}{|c|c|c|}
  \hline
  Signal Model   & Background Model & fake rate ($p_T^\gamma > 30$ GeV)\\ \hline \hline
  \multicolumn{3}{|c|}{Nominal Shapes} \\ \hline
  Breit-Wigner convoluting with crystal ball  &  $\mu$ + probe template & 2.29\% \\ \hline \hline
  \multicolumn{3}{|c|}{Alter Signal Shapes } \\ \hline
  Drell-Yan simulation convoluting with Gaussian &  $\mu$ + probe template & 2.26\% \\ \hline \hline
  \multicolumn{3}{|c|}{Alter Background Shapes } \\ \hline
  Breit-Wigner convoluting with crystal ball     &  exponential function   & 2.43\% \\ \hline
  \end{tabular}
  }
\end{table}
...
%To study the possible $p_T$ dependence of the $V\gamma$ scale factors, we perform the template fit in four lepton $p_T$ bins: (25-50, 50-70, 70-100, $>=$100) GeV. Figure \ref{fig:dphiptdependence} shows that the fluctuation of the scale factors in different $p_T$ bins are within systematic uncertainties. Since no obvious $p_T$ dependence is observed and the statistics at high $p_T$ is limited, we decide to perform the template fit in the full $p_T$ range and use a global $V\gamma$ scale factor. However, this may not be true for the fake lepton scale factors, because the $p_T$ distribution of the fake lepton background may not be perfectly modeled by the control sample. Thus we first subtract the $V\gamma$ contribution from the control sample using the global $V\gamma$ scale factors, and then use $p_T$-binned fake lepton scale factors to let the fake lepton control sample fill the control region. The $p_T$-binned scale factors are obtained by taking the ratio of the remaining events to the fake lepton templates in different $p_T$ bins. Figure \ref{fig:dphifitresult} shows the result of the fit. 
...
\hline \\
  ...
  Fast simulation $\MET$ modelling & SUSY signal           & 0 -- 30.5                     \\
\hline \\
  \end{tabular}
  }
\end{table}
